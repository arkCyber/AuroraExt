...
const WORDLIST = [
    ...
    'action', 'actor', 'actress', 'adapt', 'add', 'addict', 'address', 'adjust', 'admit', 'adult',
    'advance', 'advice', 'aerobic', 'affair', 'afford', 'afraid', 'again', 'age', 'agent', 'ago',
    ...
    'interest', 'into', 'invest', 'invite', 'involve', 'iron', 'island', 'isolate', 'issue', 'ivory',
    'jacket', 'jaguar', 'jar', 'jazz', 'jeans', 'jelly', 'jewel', 'job', 'join', 'joke',
    'journey', 'joy', 'judge', 'juice', 'jump', 'jungle', 'junior', 'junk', 'just', 'justice',
    ...
    'maid', 'mail', 'main', 'major', 'mammal', 'man', 'manage', 'mandate', 'mango', 'mansion',
    ...
    'provide', 'public', 'pudding', 'pull', 'pulp', 'pulse', 'pumpkin', 'punch', 'pupil', 'purchase',
    'purity', 'purpose', 'purse', 'push', 'put', 'puzzle', 'quality', 'quantum', 'quarter', 'question',
...
export function generateDeviceId(): string {
    ...
    const bytes = encoder.encode(infoString);
...
async function entropyToMnemonic(entropy: Uint8Array): Promise<string> {
    ...
    const checksum = hashArray[0] >>> (8 - (entropy.length * 8 / 32));
    ...
    binary += checksum.toString(2).padStart(entropy.length * 8 / 32, '0');
...
export async function generateMnemonicFromDeviceId(deviceId: string): Promise<string> {
    try {
        // 验证设备ID
        ...
        // 使用 SHA-256 生成哈希
        const hashBuffer = await crypto.subtle.digest('SHA-256', deviceIdBytes);
        const hashArray = new Uint8Array(hashBuffer);
        ...
        const checksum = checksumArray[0] >>> (8 - (entropy.length * 8 / 32));
...
export function validateMnemonic(mnemonic: string): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!mnemonic) {
        errors.push('Mnemonic cannot be empty');
    }
    ...
    if (!validateMnemonicWords(mnemonic)) {
        errors.push('One or more words are not in the BIP39 word list');
    }
...
export function validateDeviceId(deviceId: string): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
...
export function validateDeviceInfo(deviceInfo: DeviceInfo): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (!deviceInfo.hardwareConcurrency) {
        errors.push('Hardware concurrency is required');
    }
